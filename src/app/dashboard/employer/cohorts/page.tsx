...
import { Globe2, Briefcase, Clock } from 'lucide-react'
  ...
  return (
    <div>
      <div className='flex items-start justify-between mb-8'>
        <div>
          <h1 className='text-2xl font-bold text-[#0B1F3B] mb-1'>Talent Pipeline</h1>
          <p className='text-gray-500 text-sm'>Browse graduating cohorts by skills profile and graduation timeline.</p>
        </div>
        <Link
          href='/marketplace'
      ...
      <div className='grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8'>
        ...
        <StatCard Icon={Briefcase} label='Connected Talent' value={String(approvedCount)} />
      </div>
      ...
      {!cohorts || cohorts.length === 0 ? (
        ...
        <div className='grid sm:grid-cols-2 xl:grid-cols-3 gap-4'>
          {cohorts.map((cohort: Cohort) => (
            <CohortCard key={cohort.id} cohort={cohort} linkBase='/dashboard/employer/cohorts' />
          ))}
        </div>
      )}
    </div>
