...
export default async function EducationProviderCohortsPage() {
  ...
  const totalCandidates = cohorts?.reduce((sum: number, c: Cohort) => sum + c.candidate_count, 0) ?? 0

  return (
    <div>
      <div className='flex items-start justify-between mb-8'>
        <div>
          <h1 className='text-2xl font-bold text-[#0B1F3B] mb-1'>Eligible Candidate Cohorts</h1>
          <p className='text-gray-500 text-sm'>Review pathway-ready candidates by discipline. Express interest to initiate the application pipeline.</p>
        </div>
        <Link
          href='/marketplace'
      ...
      <div className='grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8'>
        ...
        <StatCard Icon={Users}         label='Total Candidates'    value={String(totalCandidates)} />
        <StatCard Icon={GraduationCap} label='Interests Expressed' value={String(interests?.length ?? 0)} />
      </div>
      ...
      {!cohorts || cohorts.length === 0 ? (
        ...
        <div className='grid sm:grid-cols-2 xl:grid-cols-3 gap-4'>
          {cohorts.map((cohort: Cohort) => (
            <CohortCard key={cohort.id} cohort={cohort} linkBase='/dashboard/education-provider/cohorts' />
          ))}
        </div>
      )}
    </div>
